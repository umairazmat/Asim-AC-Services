...
export function SocialLinks({ locale, className = "" }: SocialLinksProps) {
  return (
    <div
      ...
      aria-label={locale === "ar" ? "تابعنا على وسائل التواصل" : "Follow us on social media"}
    >
      <p className="text-[0.65rem] font-semibold tracking-wide text-white/70 uppercase sm:text-xs">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3">
        {SOCIAL_ITEMS.map((item) => (
          <a
            ...
            className={`social-link group ${item.className}`}
          >
            <item.icon className="h-4 w-4 sm:h-[1.125rem] sm:w-[1.125rem]" />
          </a>
        ))}
      </div>
    </div>
